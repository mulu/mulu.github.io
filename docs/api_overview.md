...
Mulu analyzes text on the Internet to find references to products sold by the largest online retailers such as Amazon, EBay, Overstock, Popsugar, and iTunes. The Mulu API finds and returns Mulu Product Matches that identify product mentions on review sites, magazines, or other high-value publications. 

For example, suppose a magazine mentions a lamp from a specific manufacturer in a review. Mulu detects which retailers sell that specific lamp. Online retailers then sell the lamp can add links to the magazine product review. Advertising networks can use the Mulu API to improve targeted advertising.
...
* _publisher_ - a magazine or other document publisher that generates Mulu Product Matches
...
To search for product matches, submit an [HTTP&nbsp;GET](http://tools.ietf.org/html/rfc2616#section-9.3) request to the server `api.mulu.me` and request the `/v1/matches` resource. The `/v1/matches` is the only REST API operation in the Mule API v1. Add query string parameters for your account API key and one or more search criteria.

For testing in a browser, a simple Mulu API search for product matches on a specific web site looks like:

     http://api.mulu.me/v1/matches?apiKey=APIKEY&urlPrefix=http://www.magazine.com/

Warning: Parameter values typically contain characters not permitted in query string parameters, such as ampersand and spaces. It is absolutely critical for you to use [URL encoding](http://tools.ietf.org/html/rfc3986#section-2.1) for query string parameters. 
...
| `minimumConfidence` | Optional | Floating point number from 0 to 1. Specifiy a minimum confidence value for any matches. To get more results from the Mulu API, specify a lower `minimumConfidence` value. If unspecified, the default is 0.8, which means 80% confidence of a match.  |
| `source` | Optional | Specify a Mulu product source ID to limit results only a specific product source name. Source names are case-sensitive. For example, for Amazon results set to `AMAZON`. To search multiple sources, provide the `source` parameter multiple times in your search query string. If `source` is unspecified, Mulu API searches the default set of product sources for your Mulu Account API key. (Note: If you provide multiple sources and also provide the `offer` parameter, the `offer` ID could potentially match results from multiple sources. In practice, this is unlikely because product sources use different style offer&nbsp;IDs.) |
...
#### Results: Mulu Product Match JSON

All field types are text values unless otherwise specified.

| Field | Description |
|:---+:---+:---|
| `id` | _Internal Mulu use only. Do not store or rely on this value._ |
| `product` | _JSON object_. The JSON representation of the product matched. See [Results: Product JSON](#prodFields)|
| `document` | _JSON object_. The JSON representation of the document where Mulu matched the product  [Results: Document JSON](#docFields) |
...
| `ngram` | The words Mulu matched to match the product, with extraneous words removed and the words [stemmed](http://en.wikipedia.org/wiki/Stemming) |
| `dateCrawled` | _[ISO8601](http://en.wikipedia.org/wiki/ISO_8601) format_. The most recent date Mulu confirmed this match. |
...
| `diceIndex` | _Floating point number from 0 to 1._  Dice index value, the lexical similarity of the product name and matched words on the published site. See [Confidence Levels](#confidencelevels)|
...
#### Results: Product JSON<a name="prodFields"> </a>

All field types are text values unless otherwise specified. All fields are provided and formatted by the product source data unless otherwise specified as provided by Mulu.
...
| `productPrice` | The price of the product, as a String value as provided by the product source data. The price may or may not include currency indicator such as $, before or after the number. For example, "$45.06". Also see `priceUnit`. |
| `priceUnit` | Currency code in [ISO&nbsp;4217](https://en.wikipedia.org/wiki/ISO_4217) format for this `productPrice`. For example, for US Dollars the value is `USD`. Note that the currency indicator in the `productPrice` field (such as `$`) can be different from the `priceUnit` (such as `USD`). |
| `productSource` | The Mulu product source ID for this Product Match, as determined by Mulu. For example, for Amazon the `productSource` is `AMAZON`. |
| `imageUrl` | The URL for the product image. This field may be empty. |
| `category` | The product category, for example `Clothing`. This field may be empty. |
| `upc` | The product's [Universal Product Code](https://en.wikipedia.org/wiki/Universal_Product_Code). This field may be empty. |
| `productUrl` | A URL to purchase the product. This URL is not necessarily on the same domain as the data source or the merchant (see `merchantName`).  |
...
| `offerId` | A product's unique product or offer ID as specified by a product source. For example, an Amazon Standard Identification Number ([ASIN](https://en.wikipedia.org/wiki/Amazon_Standard_Identification_Number)) |
| `type` | The product's Mulu Category Code as determined by Mulu. Current values are `SONG` (a song), `ALBUM` (an album), `ARTIST` (a music artist), and `PRODUCT` (non-music product). |

#### Results - Published Document JSON<a name="docFields"></a>

All field types are text values.
...
The following are some example Mulu API requests. To make it easier to understand examples, demo queries are shown with newlines and without URL encoding of parameters. For real queries you must use [URL&nbsp;encoding](http://tools.ietf.org/html/rfc3986#section-2.1) for all query string parameters.
